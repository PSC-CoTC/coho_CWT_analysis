...
y <- inner_join(
  x |> select(FisheryID, recovery_location_code),
  rmis_rr, by = "recovery_location_code"
)

y |> count(FisheryID) |> ggplot(aes(FisheryID, n)) + geom_col()

y |> count(FisheryID, release_location_rmis_region) |> ggplot(aes(FisheryID, n, fill = release_location_rmis_region)) + geom_col()

y |> 
  unite("rel_st_reg_bsn", c(release_location_state, release_location_rmis_region, release_location_rmis_basin)) |> 
  count(FisheryID, run_year, rel_st_reg_bsn) |> 
...
y_lm_yr <- y |> 
  ...
  count(FisheryID, run_year, rel_st_reg) |> 
  filter(FisheryID %in% c(91)) |> 
  filter(n > 4) |> 
  nest_by(FisheryID, rel_st_reg) |>
  filter(nrow(data) > 2) |> 
...
(
  ypct |>   
    ...
    labs(title = "Coho FRAM FisheryID 91: WA MA5")
) + (
  ypct |>   
    filter(FisheryID %in% c(91)) |> 
    filter(rel_st_reg %in% (ypct_lm_yr |> filter(p.value < 0.15) |> pluck("rel_st_reg"))) |> 
    ...
    geom_text(aes(label=paste("lm pval =", p.value)), data = ypct_lm_yr |> filter(p.value < 0.15), x = 1998, y = 0.5, color = 1) +
    facet_wrap(~FisheryID+rel_st_reg, scales = "fixed", labeller = label_wrap_gen(multi_line = F)) +
    labs(
