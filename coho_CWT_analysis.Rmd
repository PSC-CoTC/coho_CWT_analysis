...
Revising and generalizing to additional stock units
...
# rmis_rr |> drop_na(StockID) |> count(StockID, run_year) |> pivot_wider(names_from = run_year, values_from = n) 
# # |> writexl::write_xlsx("T:/DFW-Salmon Mgmt Modeling Team - General/Southern Fund Projects/Coho FRAM MSM CWT analysis/temp_CCT_mapped_stockID_by_year.xlsx")
...
#parallel the FRAM object structure, joining escapement as a col
er_rmis <- full_join(
  rmis_rr |> 
    drop_na(estimated_number, StockID) |>
...
er_rmis_focal |> 
  ...
  ggplot(aes(run_year, value, fill = name)) + geom_col() + facet_wrap(~StockID, scales = "free")
...
#year-stock-fishery levels in FRAM, missing in RMIS
#this is mortality being assigned in the absence of current recoveries
#which may still be valid impacts, but are not supported by recent observations...
er_fram_rmis |> 
...
#year-stock-fishery levels in RMIS, missing in FRAM
#this is mortality NOT being assigned despite the presence of current recoveries
er_fram_rmis |> 
...
#   distinct(sidnm, run_year, fram_catch = fram_landed_yr_sum, fram_escp, rmis_catch = rmis_est_num_yr_sum, rmis_escp) |> 
#   pivot_longer(-c(run_year, sidnm)) |> 
#   separate(name, c("f_r", "var"), sep = "_", extra = "merge") |> 
#   unite("year_source", run_year, f_r) |> 
#   ggplot(aes(year_source, value, fill = var)) +
...
```{r fram_rmis_diff_means}
fram_rmis |> 
  drop_na(pseudo_er) |> 
  group_by(sidnm, fidnm) |> 
  summarise(across(c(fram, rmis, fram_rmis), list(mean=mean)), .groups = "drop") |> 
