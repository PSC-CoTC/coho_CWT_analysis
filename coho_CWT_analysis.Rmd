...
lu <- list(
  ...
  # # #COPY NOW preserved in sheet "cct_mapping" of rmis_loc_fram_fishery.xlsx
  cwt_stk_orig = readr::read_csv(file.path(dir_proj, "MSMDB/MSM_Stock_CWT2.csv")) |>
  ...
  ,
  cwt_stk = readr::read_csv("lu_cct_MSM_Stock_CWT07112022.csv") |>
  ...
  ,
  #AHB file...
  ahb_fishmap = readr::read_csv("lu_ahb_FishMapJun29_2022.csv") |> 
...
# fram$fishery |>
#   filter(str_detect(FisheryTitle, "Sport|Spt")) |>
#   print(n=nrow(fram$fishery))
...
#add CCT mapped StockIDs to dataset
#rmis_rr <- rmis_rr |> left_join(lu$cwt_stk_orig, by = c("tag_code", "run_year"))
rmis_rr <- rmis_rr |> left_join(lu$cwt_stk, by = c("tag_code"))

#see section "AHB fishmap" below for FisheryID mapping
```

```{r cct_stock_map_update_qaqc}
# foo = left_join(
#   rmis_rr |> select(uid, run_year, tag_code, estimated_number, StockID),
#   lu$cwt_stk_orig |> select(run_year, tag_code, StockID_old = StockID),
#   by = c("run_year", "tag_code")
#   )
# 
# foo |>
#   drop_na(estimated_number) |> #excluded in later analysis 
#   drop_na(StockID_old) |> #exclude recoveries of releases that were not previously mapped to stock
#   filter(is.na(StockID)) |> #focus on now-missing mapping
#   #these are recoveries that were previously mapped to stocks but are now not...
#   count(StockID_old, run_year) |> pivot_wider(names_from = run_year, values_from = n) |> print(n=100)
# 
# foo |> filter(StockID_old==48, run_year==2020)
# 
# rmis_rr |> filter(uid==2798366)
# 
# lu$cwt_stk |> filter(StockID==48)
# lu$cwt_stk |> filter(tag_code=="055904")
# lu$cwt_stk_orig |> filter(tag_code=="055904")
# 
# lu$cwt_stk |> filter(tag_code=="055751")
# lu$cwt_stk_orig |> filter(tag_code=="055751")
# 
# lu$cwt_stk |> filter(nchar(tag_code) > 6)
# 
# lu$cwt_stk |> filter(tag_code=="633337")
# lu$cwt_stk_orig |> filter(tag_code=="633337")
...
```{r catch_sample_to_FisheryID}
cs <- readr::read_csv("T:/DFW-Salmon Mgmt Modeling Team - General/Southern Fund Projects/Coho FRAM MSM CWT analysis/MarkRateA/Data for MR_R/RMIS2010_20CohoMrkSample.csv") |> 
  rowid_to_column(var = "uid")
  

cs_ahb_map <- cs |> 
  select(uid, catch_location_code, fishery) |> 
  mutate(fishery_single = floor(fishery/10)) 
...
cs_ahb_map <- map_df(
  ...
  ~inner_join(
    cs_ahb_map |> 
      mutate(PSC_Code = str_sub(catch_location_code, 1, .x) |> str_pad(width = .x, side = "right"))
    ,
    lu$ahb_fishmap |> filter(Num_Chars == .x) |> 
      mutate(PSC_Code = str_pad(PSC_Code, width = .x, side = "right"))
    ,
...
#some cases of same fishery from different n-chars being considered on same recovery_location_code
#some cases initially from simple row duplication in lookup, now fixed on read-in
overmapped <- cs_ahb_map |> count(uid) |> filter(n>1) |> select(uid)
...
cs <- left_join(
  ...
  #split out multiple-mapped, make distinct per UID and rebind
  bind_rows(
    anti_join(cs_ahb_map, overmapped, by = "uid") |> select(uid, FisheryID)
    ,
    semi_join(cs_ahb_map, overmapped, by = "uid") |> 
      group_by(uid) |> 
      slice_max(Num_Chars, n = 1, with_ties = F) |> 
      ungroup() |> 
      select(uid, FisheryID) #|> count(uid) |> filter(n>1)
...
rm(cs_ahb_map, overmapped)

writexl::write_xlsx(
  list(
    focal_stock_fishery_coefs = semi_join(cs, er_fram |> distinct(FisheryID), by = "FisheryID") |> 
      group_by(catch_year, FisheryID, FisheryName) |> 
      summarise(
        number_caught_sum = sum(number_caught, na.rm = T),
        number_caught_not0samp_sum = sum(number_caught[number_sampled != 0], na.rm = T),
        number_caught_0samp_sum = sum(number_caught[number_sampled == 0], na.rm = T),
        samp_catch_pct_tot_catch = number_caught_not0samp_sum / number_caught_sum,
        nosamp_catch_pct_tot_catch = number_caught_0samp_sum / number_caught_sum,
        coef_sampled = number_caught_sum / number_caught_not0samp_sum,
        .groups = "drop"
      ),
    focal_stock_fisheries = semi_join(cs, er_fram |> distinct(FisheryID), by = "FisheryID"),
    all_fisheries = cs
  ), 
  path = "T:/DFW-Salmon Mgmt Modeling Team - General/Southern Fund Projects/Coho FRAM MSM CWT analysis/MarkRateA/Data for MR_R/RMIS2010_20CohoMrkSample_qaqc_unsampled.xlsx"
)


# semi_join(cs, er_fram |> distinct(FisheryID), by = "FisheryID") |> 
#   count(number_sampled == 0) |> mutate(pct = n/sum(n))
...
focal_cols_recs <- c("recovery_id", "tag_code", "run_year", "recovery_location_code", "recovery_location_name", "fishery", "gear", "estimated_number", "recorded_mark", "catch_sample_id", "sample_type")
...
This first chunk distinguishes the unique *recovery_location_codes* along with new fields by character position, following the RMIS schema (PSC_V41_Specification_Nov2021.pdf).
...
# Skagit test of preliminary psuedo-ER comparison
...
```{r skagit_calc_postseason_fram_pseudo_er}
fram <- list()
...
# FRAM-RMIS comparison

## Extract CoTC post-season FRAM estimates

```{r er_fram}
#rmis_rr |> drop_na(StockID) |> count(StockID, Stock_Name, pr, pr_mu) |> arrange(desc(n)) |> print(n=30)
#Stilly: rmis_rr |> drop_na(StockID) |> filter(StockID %in% 29:32) |> count(StockID, Stock_Name, pr, pr_mu) |> arrange(desc(n)) |> print(n=30)
#Sno: rmis_rr |> drop_na(StockID) |> filter(StockID %in% 35:38) |> count(StockID, Stock_Name, pr, pr_mu) |> arrange(desc(n)) |> print(n=30)
...
focal_stock_ids <- c(20, 38, 48, 58, 68, 96, 134, 148, 164, 166, 176, 226, 230)
#per AHB request to combine NT & Tr net fisheries
paired_fisheries <- c(80, 82, 87, 96, 101, 109, 111, 119, 121, 123, 
                      130, 132, 137, 139, 141, 143, 145, 153, 155, 157, 159)

# fram$fishery |> 
#   mutate(
#     FisheryID_p = if_else(FisheryID %in% paired_fisheries, FisheryID+1, as.double(FisheryID)),
#     FisheryName_p = if_else(FisheryID %in% c(paired_fisheries, paired_fisheries+1),
...
er_fram <- full_join(
  fram$mort |>
    ...
    summarise(
      across(c(LandedCatch, MSFLandedCatch), sum),
      fram_landed_yr = LandedCatch + MSFLandedCatch, .groups = "drop") |> 
    group_by(RunYear, StockID, StockLongName) |> 
    mutate(
      LandedCatch = NULL, MSFLandedCatch = NULL,
      across(fram_landed_yr, list(sum = sum), .names = "{.col}_{.fn}")
      ) |> 
    ungroup()
  ,
  fram$escp |> 
  ...
  mutate(
    run_year = as.integer(RunYear), RunYear = NULL,
    fram_abund = fram_landed_yr_sum + fram_escp,
    fram_er_fishery = fram_landed_yr / fram_abund,
    fram_er = fram_landed_yr_sum / fram_abund
    #across(c(fram_landed_yr, fram_landed_yr_sum), list(fram_er = ~.x/fram_abund))
  ) |> 
  select(run_year, everything())

er_fram_no_fwspt <- full_join(
  fram$mort |>
    filter(!(FisheryID %in% fram_fishery_fw_spt)) |> 
    group_by(RunYear, StockID, StockLongName, FisheryID, FisheryName) |> 
    summarise(
      across(c(LandedCatch, MSFLandedCatch), sum),
      fram_landed_yr = LandedCatch + MSFLandedCatch, .groups = "drop") |> 
...
```{r rmis_qaqc}
# rmis_rr |> 
#   count(FisheryID, FisheryName, recovery_location_code, fishery, gear, recovery_location_name) |>
#   writexl::write_xlsx("T:/DFW-Salmon Mgmt Modeling Team - General/Southern Fund Projects/Coho FRAM MSM CWT analysis/qaqc_AHB_mapped_FisheryID_rec_loc_code.xlsx")

# #added as sheet "cct_mapped_stockID_3s_no_escp"
# #then again as "cct_stockID_3s_noescp_Jun29"
# rmis_rr |>
#   drop_na(estimated_number, StockID) |>
#   filter(age == 3, f_type != "escp", between(run_year, 2010, 2020)) |>
#   count(FisheryID, FisheryName, recovery_location_code, fishery, gear, recovery_location_name) |>
#   writexl::write_xlsx("C:/Users/auerbdaa/Downloads/temp.xlsx")

# rmis_rr |> 
#   filter(age == 3, f_type != "escp", between(run_year, 2010, 2020)) |> 
#   filter(is.na(StockID)) |> 
#   filter(cwt_1st_mark %in% c("5000", "5500")) |> 
#   count(release_location_state, release_location_rmis_region, release_location_rmis_basin,
#         release_location_name, hatchery_location_name, stock_location_name, 
#         cwt_1st_mark, cwt_2nd_mark,
#         tag_code) |> 
#   arrange(desc(n)) |> #print(n=30)
#   writexl::write_xlsx("T:/DFW-Salmon Mgmt Modeling Team - General/Southern Fund Projects/Coho FRAM MSM CWT analysis/qaqc_CCT_unmapped.xlsx")

# #first export was including escp as temp_CCT_mapped_stockID_by_year.xlsx
# writexl::write_xlsx(
#   x = list(
#     no_escp = rmis_rr |> drop_na(StockID) |> 
#       filter(f_type != "escp") |> 
#       count(StockID, run_year) |> pivot_wider(names_from = run_year, values_from = n) ,
#     with_escp = rmis_rr |> drop_na(StockID) |> 
#       count(StockID, run_year) |> pivot_wider(names_from = run_year, values_from = n) 
#   ), 
#   path = "T:/DFW-Salmon Mgmt Modeling Team - General/Southern Fund Projects/Coho FRAM MSM CWT analysis/qaqc_CCT_mapped_stockID_by_year.xlsx"
#   )

#unmapped fishery recoveries post-2010 appear relatively inconsequential?
rmis_rr |> 
  filter(age == 3, f_type != "escp", between(run_year, 2010, 2020)) |> 
  drop_na(StockID) |>
  drop_na(estimated_number) |> 
  filter(is.na(FisheryID)) |> arrange(desc(estimated_number))

# rmis_rr |> 
#   filter(
#     age == 3,
#     f_type != "escp",
#     between(run_year, 2010, 2020),
#     !is.na(StockID),
#     !is.na(estimated_number),
#     !is.na(FisheryID)
#     ) |> 
#   count(
#     StockID, Stock_name, FisheryID, FisheryName, FisheryTitle
#   ) |> 
#   writexl::write_xlsx("qaqc_total_recs_by_stock_fishery.xlsx")


# rmis_rr |> filter(is.na(estimated_number) | estimated_number == 0) |> count(run_year, estimated_number) |> mutate(estimated_number = as.character(estimated_number), estimated_number = replace_na(estimated_number, "NA")) |> ggplot(aes(run_year, n, fill = estimated_number)) + geom_col()

```

## Construct RMIS estimates paralleling FRAM

```{r er_rmis}
#filter logic excludes RMIS fishery values > 59 
#these evaluate f_type==NA and fail both f_type=="escp" and f_type!="escp"
er_rmis <- full_join(
  #analogous to FRAM mortality table
  rmis_rr |> 
    drop_na(estimated_number, StockID) |> 
    filter(age == 3, f_type != "escp") |> 
    mutate(
    ...
    group_by(run_year, StockID, f_type, FisheryID, FisheryName) |> 
    summarise(rmis_est_num_yr = sum(estimated_number), .groups = "drop") |> 
    group_by(run_year, StockID) |> 
    mutate(rmis_est_num_yr_sum = sum(rmis_est_num_yr)) |> 
    ungroup()
  ,
  ...
  mutate(
    ...
    rmis_abund = rmis_est_num_yr_sum + rmis_escp,
    rmis_er_fishery = rmis_est_num_yr / rmis_abund,
    rmis_er = rmis_est_num_yr_sum / rmis_abund
  )

#need additional/alternative filter criteria?
#large majority of records are Buoy10 and Lwr ColR, with appreciable Fraser & Chehalis Spt
er_rmis_no_fwspt <- full_join(
  #analogous to FRAM mortality table
  rmis_rr |> 
    drop_na(estimated_number, StockID) |> #1096098 only 3s, 1117270 including more than 3s
    filter(age == 3, f_type != "escp") |> #548455
    filter(!(f_type == "sport" & rlwtr == "F")) |> 
    mutate(
      StockID = if_else(StockID == 18, 20, StockID),
      FisheryID = if_else(FisheryID %in% paired_fisheries, FisheryID+1, as.double(FisheryID)),
      FisheryName = if_else(FisheryID %in% c(paired_fisheries, paired_fisheries+1),
  ...
  rmis_rr |> 
    drop_na(estimated_number, StockID) |> #1096098 only 3s, 1117270 including more than 3s
    filter(age == 3, f_type == "escp") |> #531353
    mutate(StockID = if_else(StockID == 18, 20, StockID)) |> 
    group_by(run_year, StockID) |> 
    summarise(rmis_escp = sum(estimated_number), .groups = "drop")
  ,
...
#reduce these all-stock objects to those in FRAM dataset
er_rmis_selected_stocks <- er_rmis |> 
  filter(
    StockID %in% unique(er_fram$StockID),
    between(run_year, min(er_fram$run_year), max(er_fram$run_year))
  )

er_rmis_selected_stocks_no_fwspt <- er_rmis_no_fwspt |> 
  filter(
    StockID %in% unique(er_fram$StockID),
    between(run_year, min(er_fram$run_year), max(er_fram$run_year))
  )
...
Why is Sno missing net recoveries in 2019? Checked and appear to be missing in RMIS...AHB seeing mostly "net" in 8A/8D in other years

```{r er_rmis_eda_qaqc}
#simple time series of annually summed fishery and escapement estimated_number values
er_rmis_selected_stocks |> 
  distinct(run_year, StockID, rmis_est_num_yr_sum, rmis_escp) |> 
  pivot_longer(-c(run_year, StockID)) |> 
  ggplot(aes(run_year, value, fill = name)) + geom_col() + scale_fill_grey() + facet_wrap(~StockID, scales = "free")
#simple time series of annual "ER" of fishery est_num / (fishery + escapement est num)
er_rmis_selected_stocks |> 
  distinct(run_year, StockID, rmis_er) |> 
  ggplot(aes(run_year, rmis_er)) + geom_col() + facet_wrap(~StockID, scales = "free")

```

## Join FRAM & RMIS

Now excluding 2020 as seemingly still too incomplete?

```{r fram_rmis}

#left wide and verbose...
er_fram_rmis <- full_join(
  er_fram |> select(-StockLongName, -FisheryName),
  er_rmis_selected_stocks |> select(-FisheryName),
  ...
  mutate(
    ...
    d_landed_est_num_yr = fram_landed_yr - rmis_est_num_yr,
    d_landed_est_num_yr_sum = fram_landed_yr_sum - rmis_est_num_yr_sum,
    d_escp_fram_rmis = fram_escp - rmis_escp,
    d_er_fram_rmis_fishery = fram_er_fishery - rmis_er_fishery,
    d_er_fram_rmis = fram_er - rmis_er
    ) 

er_fram_rmis_no_fwspt <- full_join(
  er_fram_no_fwspt|> select(-StockLongName, -FisheryName),
  er_rmis_selected_stocks_no_fwspt |> select(-FisheryName),
  by = c("run_year", "StockID", "FisheryID")
) |>
  left_join(fram$mort |> distinct(StockID, StockLongName), by = "StockID") |> 
  left_join(fram$mort |> distinct(FisheryID, FisheryName), by = "FisheryID") |> 
...
## Summed mortality of year-stock-fishery levels in FRAM, missing in RMIS

This is mortality being assigned in the absence of current recoveries. These may be valid impacts, but are not supported by recent observations.

```{r fram_rmis_missing_in_rmis}
er_fram_rmis |> 
  filter(is.na(rmis_est_num_yr)) |>
  group_by(sidnm, fidnm) |> summarise(across(fram_landed_yr, sum), .groups = "drop") |> #ggplot() + stat_ecdf(aes(fram_landed_yr, color = sidnm))
  filter(fram_landed_yr >= 100) |> 
  ggplot(aes(fidnm, fram_landed_yr)) +
  geom_col() + coord_flip() + facet_wrap(~sidnm, scales = "free") +
  labs(
    title = "Sum of FRAM landed catch estimate 2010-2020 for fisheries missing RMIS recoveries in some or all years",
    subtitle = "Stock-fishery combinations with FRAM summed LC >= 100")
```

## Summed mortality of year-stock-fishery levels in RMIS, missing in FRAM

This is mortality *not* being assigned despite the presence of current recoveries. The overall magnitude appears small for the WA PS examined.

```{r fram_rmis_missing_in_fram}
#er_fram_rmis |> 
er_fram_rmis_no_fwspt |> 
  filter(is.na(fram_landed_yr)) |> 
  arrange(desc(rmis_est_num_yr)) |> 
  select(run_year, sidnm, fidnm, rmis_est_num_yr:rmis_er) |> 
  group_by(sidnm, fidnm) |> summarise(across(rmis_est_num_yr, sum), .groups = "drop") |> #ggplot() + stat_ecdf(aes(rmis_est_num_yr, color = sidnm))
  ggplot(aes(fidnm, rmis_est_num_yr)) +
  geom_col() + coord_flip() + facet_wrap(~sidnm, scales = "free_y") +
  labs(
    title = "Sum of RMIS estimated number 2010-2020 for fisheries missing FRAM mortality in some or all years",
    )

```

## RMIS total estimated number > FRAM landed catch

Only a few instances. Would not expect to see this barring sampling expansion issues?

```{r}
er_fram_rmis_no_fwspt |> 
  filter(rmis_est_num_yr_sum > fram_landed_yr_sum) |> 
  count(sidnm, run_year) 
```


## Catch:escapement ratios

Compare the annual ratio of total fishery recoveries to total escapement recoveries.

```{r er_fram_rmis_catch_escp_ratio_bars}
#these are annual totals, already previously summed separately for each source
#initial drop_na() just limits to actual values
er_fram_rmis |> 
  drop_na(fram_er, rmis_er) |> 
  distinct(sidnm, run_year, fram_landed_yr_sum, fram_escp, rmis_est_num_yr_sum, rmis_escp) |> 
  mutate(
    `fram_catch:escp` = fram_landed_yr_sum / fram_escp,
    `rmis_catch:escp` = rmis_est_num_yr_sum / rmis_escp
  ) |> 
  select(run_year, sidnm, contains("_catch:")) |> 
  pivot_longer(-c(run_year, sidnm)) |> 
  separate(name, c("source", "var"), sep = "_", extra = "merge") |> 
  ggplot(aes(run_year, value, fill = source)) +
  geom_col(position = position_dodge()) +
  facet_wrap(~sidnm, scales = "free")

```

## ER comparison including FW sport

```{r gt_total_er_mean}
er_fram_rmis |> 
  drop_na(fram_er, rmis_er) |>
  group_by(sidnm) |> 
  distinct(sidnm, run_year, fram_er, rmis_er, d_er_fram_rmis) |> #arrange(sidnm) |> print(n=50)
  summarise(across(c(fram_er, rmis_er, d_er_fram_rmis), mean)) |> 
  gt()
```
...
er_fram_rmis |> 
  ...
  group_by(sidnm) |> 
  distinct(sidnm, run_year, fram_er, rmis_er, d_er_fram_rmis) |> #arrange(sidnm) |> print(n=50)
  summarise(across(c(fram_er, rmis_er), mean)) |> 
  pivot_longer(-sidnm) |> 
  ggplot(aes(sidnm, value, fill = name)) + 
  geom_col(position = position_dodge()) +
  geom_text(aes(y = -0.07, label = scales::percent(round(value, 3))), position = position_dodge2(width = 0.7)) +
  scale_x_discrete("") +
  scale_y_continuous("Pseudo-ER", limits = c(-0.1, 1),labels = scales::percent) +
  coord_flip() +
  scale_fill_grey("") +
  theme(legend.position = "top") +
  labs(
    title = "Mean 2010-2020 total pseudo-ER, FRAM - RMIS"
  )
...
er_fram_rmis |> 
  drop_na(fram_er, rmis_er) |>
  group_by(sidnm) |> 
  distinct(sidnm, run_year, fram_er, rmis_er, d_er_fram_rmis) |>
  ungroup() |>
  gt(groupname_col = "sidnm", rowname_col = "run_year") |> 
  fmt_percent(contains("_er"))
```

```{r gg_total_er_annual_diff}
er_fram_rmis |> 
  drop_na(fram_er, rmis_er) |>
  group_by(sidnm) |> 
  distinct(sidnm, run_year, fram_er, rmis_er, d_er_fram_rmis) |>
  ungroup() |>
  ggplot(aes(run_year, d_er_fram_rmis)) + 
  geom_col() +
  scale_y_continuous("", labels = scales::percent) +
  facet_wrap(~sidnm, scales = "free_y", labeller = label_wrap_gen(multi_line = F), ncol = 1) +
  labs(
    title = "Differences in annual total pseudo-ER, FRAM - RMIS"
  )
```

```{r gg_total_er_annual_bars}
er_fram_rmis |> 
  drop_na(fram_er, rmis_er) |>
  group_by(sidnm) |> 
  distinct(sidnm, run_year, fram_er, rmis_er) |>
  ungroup() |>
  pivot_longer(-c(sidnm, run_year)) |> 
  ggplot(aes(run_year, value, fill = name)) + 
  geom_col(position = position_dodge()) +
  scale_fill_grey("") +
  facet_wrap(~sidnm, scales = "free", ncol = 2)
    
```

```{r gg_fishery_er_diff_boxplots}
min_diff_er <- 0.01 #annual, per-fishery

#for stock-fishery-year levels with both sources present...
er_fram_rmis |> 
  drop_na(fram_er, rmis_er) |> 
  filter(abs(d_er_fram_rmis_fishery) >= min_diff_er) |> 
  ggplot(aes(fidnm, d_er_fram_rmis_fishery)) +
  geom_boxplot(outlier.shape = NA) +
  geom_text(aes(label = run_year), size = 2, position = "jitter", alpha = 0.4) +
  geom_hline(yintercept = 0) +
  coord_flip() +
  ...
  labs(
    ...
    subtitle = paste("Minimum difference >=", min_diff_er)
  )

```

```{r gg_fishery_er_diff_means}
er_fram_rmis |> 
  drop_na(fram_er, rmis_er) |> 
  group_by(sidnm, fidnm) |> 
  summarise(across(c(fram_er_fishery, rmis_er_fishery, d_er_fram_rmis_fishery), list(mean=mean)), .groups = "drop") |> 
  filter(abs(d_er_fram_rmis_fishery_mean) >= min_diff_er) |> 
  ggplot(aes(fidnm, d_er_fram_rmis_fishery_mean, fill = d_er_fram_rmis_fishery_mean > 0)) +
  geom_col(show.legend = F) +
  scale_fill_manual(values = c("purple", "gold")) +
  facet_wrap(~sidnm, scales = "free_x", ncol=1) +
  labs(
    title = "Mean 2010-2020 difference FRAM - RMIS pseudo ERs",
    subtitle = "gold/above 0: FRAM landed/(landed+escp) > RMIS fishery/(fishery+escp)"
  )

```

## ER comparison excluding FW sport

```{r gt_total_er_mean_no_fwspt}
er_fram_rmis_no_fwspt |> 
  drop_na(fram_er, rmis_er) |>
  group_by(sidnm) |> 
  distinct(sidnm, run_year, fram_er, rmis_er, d_er_fram_rmis) |> #arrange(sidnm) |> print(n=50)
...
```{r gg_total_er_annual_diff_no_fwspt}
#same as preceding table, as plot
er_fram_rmis_no_fwspt |> 
  drop_na(fram_er, rmis_er) |>
  group_by(sidnm) |> 
  distinct(sidnm, run_year, fram_er, rmis_er, d_er_fram_rmis) |>
  ...
  geom_col() +
  scale_y_continuous("", labels = scales::percent) +
  facet_wrap(~sidnm, scales = "free_y", labeller = label_wrap_gen(multi_line = F), ncol = 2) +
  labs(
    title = "Differences in annual total pseudo-ER, FRAM - RMIS"
  )
```

```{r gg_total_er_annual_bars_no_fwspt}
er_fram_rmis_no_fwspt |> 
  drop_na(fram_er, rmis_er) |>
  group_by(sidnm) |> 
  distinct(sidnm, run_year, fram_er, rmis_er) |>
...
#   group_by(sidnm) |> 
#   distinct(sidnm, run_year, fram_er, rmis_er) |>
#   summarise(across(contains("_er"), list(mean = ~mean(.), var = ~var(.))))

```

```{r gg_fishery_er_diff_boxplots_no_fwspt}
min_diff_er <- 0.01 #annual, per-fishery
...
```{r gg_fishery_er_diff_means_no_fwspt}
er_fram_rmis_no_fwspt |> 
  drop_na(fram_er, rmis_er) |> 
  #filter(StockID < 165) |> 
  group_by(sidnm, fidnm) |> 
  ...
  facet_wrap(~sidnm, scales = "free_x", ncol=2) +
  labs(
    title = "Mean 2010-2020 difference FRAM - RMIS pseudo ERs",
    subtitle = "gold/above 0: FRAM landed/(landed+escp) > RMIS fishery/(fishery+escp)"
  )
...
er_fram_rmis_no_fwspt |>
  filter(fram_landed_yr >= min_fram_landed, run_year %in% c(2010:2014, 2017:2019)) |> 
  mutate(
    shp = case_when(
      d_er_fram_rmis_fishery > 0 ~ "FRAM > RMIS",
      d_er_fram_rmis_fishery < 0 ~ "FRAM < RMIS",
      d_er_fram_rmis_fishery == 0 ~ "FRAM = RMIS",
      is.na(rmis_er_fishery) ~ "FRAM-only",
      is.na(fram_er_fishery) ~ "RMIS-only"
    ),
    #sze = abs(d_er_fram_rmis_fishery)
    sze = if_else(!is.na(d_er_fram_rmis_fishery), abs(d_er_fram_rmis_fishery), 0)
    ) |> 
  ggplot(aes(sidnm, fidnm)) +
  geom_point(aes(shape = shp), size = 2) +
  geom_point(aes(shape = shp, size = sze, fill = d_er_fram_rmis_fishery)) +
  scale_shape_manual(name = "", values = c("FRAM > RMIS" = 24, "FRAM < RMIS" = 25, "FRAM = RMIS" = 22, "FRAM-only" = 20, "RMIS-only" = 3)) +
  scale_size("ER diff. abs.", range = c(2.5, 7.5)) +
  scale_fill_gradient2(name = NULL, low = "purple", mid = "lightgrey", high = "orange", midpoint = 0, na.value = "pink", aesthetics = c("color", "fill")) +
  guides(fill = guide_colorbar(title="ER difference")) +
  scale_x_discrete("", guide = guide_axis(n.dodge = 2)) +
  scale_y_discrete("") +
  facet_wrap(~run_year, ncol = 2) +
  labs(
    title = "Differences in annual per-fishery pseudo-ER, FRAM - RMIS",
    subtitle = paste("Minimum annual landed catch in FRAM >=", min_fram_landed)
  )
# + transition_states(states = run_year, state_length = 1, transition_length = 1) 
# #+ annotate("text", label = "{closest_state}")
```
...
gg_dif <- map(
  set_names(levels(er_fram_rmis_no_fwspt$sidnm)),
  function(s) {
    d <- er_fram_rmis_no_fwspt |>
      filter(
        sidnm == s,
        fram_landed_yr >= min_fram_landed
    ...
    bind_rows(
      ...
      d |> group_by(sidnm, fidnm) |> 
        summarise(
          run_year = "mean error",
          d_er_fram_rmis_fishery = mean(d_er_fram_rmis_fishery, na.rm=T), .groups = "drop"),
      ...
      mutate(
        fidnm = factor(fidnm, levels = c(levels(er_fram_rmis_no_fwspt$fidnm), "0: annual mean error")),
        shp = case_when(
          d_er_fram_rmis_fishery > 0 ~ "FRAM > RMIS",
          d_er_fram_rmis_fishery < 0 ~ "FRAM < RMIS",
          d_er_fram_rmis_fishery == 0 ~ "FRAM = RMIS",
...
er_fram_rmis_no_fwspt |> 
  drop_na(fram_er, rmis_er) |> 
  filter(StockID %in% c(20, 38, 96)) |> 
  filter(FisheryID==91) |> 
  group_by(sidnm, fidnm) |> 
  summarise(across(contains("_er"), list(mean = ~mean(.), var = ~var(.), sd = ~sd(.))))
...
map(
  c(20, 38),
  ~er_fram_rmis_no_fwspt |> 
    filter(StockID == .x, FisheryID %in% fram_ps_spt[-c(7,8)]) |> 
    select(run_year, sidnm, fidnm, fram_er_fishery, rmis_er_fishery) |> 
    pivot_longer(cols = contains("_er")) |> 
    ggplot(aes(run_year, value, fill = name)) +
    geom_col(position = position_dodge()) +
    scale_x_continuous("", breaks = 2010:2020, guide = guide_axis(angle = 90)) +
    scale_y_continuous("", labels = scales::percent) +
    stat_smooth(aes(color = name), method="lm", formula=y~1, se=FALSE) + 
    scale_fill_grey(aesthetics = c("fill", "color")) +
    facet_wrap(~fidnm, scales="free", nrow = 2, labeller = label_wrap_gen(multi_line = F)) +
    labs(subtitle = paste0("FRAM Stock", .x))
) |> 
...
## examine annual percentages of recoveries

next bootstrap years to study (in)stability in the avg ERs that would be calc'd?

```{r}

# rlsp=="3" & rlwtr=="M" & rlsctr=="1" & rlrgn=="11" & rlarea=="06  " ~ 92, #Area 6 Spt	WA Area 6 Sport (Port Angeles)
# rlsp=="3" & rlwtr=="M" & rlsctr=="1" & rlrgn=="11" & rlarea=="07  " ~ 93


#Area 5 Spt	WA Area 5 Sport (Sekiu); includes various terminal zones that perhaps could/should assign elsewhere
#but identical number of recoveries as AHB mapped: rmis_rr |> filter(FisheryID==91)
#rmis_rr |> filter(FisheryID == 91) |> count(FisheryID, recovery_location_code, recovery_location_name) |> arrange(desc(n))
rmis_rr_a5 <- rmis_rr |> 
  filter(
    (rlsp=="3" & rlwtr=="M" & rlsctr=="1" & rlrgn=="11" & rlarea=="05  "),
    age == 3,
    between(as.numeric(recorded_mark), 5000, 5500),
    between(as.numeric(cwt_1st_mark), 5000, 5500)
    )

#no stark trend in number of recoveries (or estimated number) 
rmis_rr_a5 |> 
  group_by(run_year) |> 
  summarise(n_rec_year = n(), est_num_year = sum(estimated_number), .groups = "drop") |> 
  pivot_longer(cols = -run_year) |> 
  ggplot(aes(run_year, value)) + geom_line(aes(color = name)) + geom_smooth() + facet_wrap(~name, scales = "free", ncol = 1)

#good diversity of hatcheries+stocks, line up well with CCT mapping
rmis_rr_a5 |> 
  count(hatchery_location_name, stock_location_name, StockID) |> arrange(desc(n)) |> print(n=50)
#and example indicating that where unmapped, mostly due to years when tag codes not assessed...
rmis_rr_a5 |> 
  filter(hatchery_location_name == "QUILCENE NFH", 
         is.na(StockID)) |> 
  count(hatchery_location_name, stock_location_name, StockID, run_year) |> arrange(desc(n)) |> print(n=50)
  

# #calculate per-hatchery ("stock") percentages of recoveries per-year
# rmis_rr_a5_pct <- rmis_rr_a5 |> 
#   mutate(release_st_rgn_bsn = paste(release_location_state, release_location_rmis_region, release_location_rmis_basin, sep = "_")) |> 
#   group_by(run_year, release_location_state, release_location_rmis_region, release_location_rmis_basin, release_st_rgn_bsn, hatchery_location_name) |> 
#   summarise(
#     n_rec_year = n(), 
#     est_num_year = sum(estimated_number),
#     .groups = "drop"
#     ) |> 
#   group_by(run_year) |> 
#   mutate(
#     across(c(n_rec_year, est_num_year), list(total = ~sum(.))),
#     n_rec_pct = n_rec_year / n_rec_year_total,
#     est_num_pct = est_num_year / est_num_year_total
#     ) |> 
#   ungroup()
# 
# rmis_rr_a5_pct |> 
#   drop_na(hatchery_location_name) |> #count(n_rec_year<3)
# #  filter(n_rec_year >= 3) |> 
#   ggplot(aes(run_year, n_rec_pct, color = paste(release_location_state, release_location_rmis_region, sep = "_"))) +
# #  geom_line()
#   geom_smooth(method = "lm", se = F)
# 
# 
# #3d points/lines
# #leaving out add_lines re-orders hatcheries to clearly show those that appear later in the record
# #including add_lines orders x-axis alphabetically
# rmis_rr_a5_pct |> 
#   drop_na(hatchery_location_name) |>
#   select(run_year, hatchery_location_name, n_rec_pct) |> 
#   plotly::plot_ly(x = ~hatchery_location_name, y = ~run_year, z = ~n_rec_pct, color = ~n_rec_pct) |> 
#   plotly::add_lines() |> 
...
# #   drop_na(hatchery_location_name) |>
# #   select(run_year, hatchery_location_name, n_rec_pct) |> 
# #   pivot_wider(names_from = run_year, values_from = n_rec_pct) |> 
# #   mutate(across(-hatchery_location_name, ~replace_na(.,0))) |> 
# #   column_to_rownames("hatchery_location_name") |> 
# #   slice(1:7) |> select(1:7) %>% 
# #   plotly::plot_ly(z = ., type = "surface")
# 
# 
# #not ER, but...trailing means of annual percentage of total recoveries
# {rmis_rr_a5_pct |> 
#   drop_na(hatchery_location_name) |>
#   filter(release_location_state %in% c("WA", "BC")) |> 
#   #filter(str_detect(hatchery_location_name, "QUILCENE")) |> 
#   group_by(hatchery_location_name) |> 
#   mutate(
#     mu6 = slider::slide_dbl(n_rec_pct, ~mean(., na.rm=T), .before = 6)
#   ) |> 
#   ggplot(aes(run_year, mu6, color = paste(release_location_state, release_location_rmis_region, sep = "_"), group = hatchery_location_name)) +
#   geom_line() + scale_color_discrete("") +
#   facet_wrap(~release_location_state+release_location_rmis_region) } |> 
#   plotly::ggplotly()
# 
# rmis_rr_a5_pct |> 
#   drop_na(hatchery_location_name) |>
...
#     mu6 = slider::slide_dbl(n_rec_pct, ~mean(., na.rm=T), .before = 6)
#   ) |> 
#   plotly::plot_ly(x = ~hatchery_location_name, y = ~run_year, z = ~mu6, color = ~mu6) |> 
#   plotly::add_lines() |> 
#   plotly::add_markers(size = 1)


rmis_rr_a5 |> 
  #filter(str_detect(hatchery_location_name, "QUILCENE"), run_year == 1990) |> 
  #calc per-recovery percentage of release count, normalizing for variation in release/tag numbers
  #this value incorporates a host of non-fishing survival factors determining whether releases made it to zone where could be caught & sampled
  mutate(
    release_st_rgn = paste(release_location_state, release_location_rmis_region, sep = "_"),
    #pct_cwt_rec = 1 / cwt_1st_mark_count,
    pct_cwt_est_num = estimated_number / cwt_1st_mark_count
    ) |> 
  #now aggregate these per-recovery "percentage of release" over individual recoveries within a tag_code
  group_by(
    run_year, 
  ...
  summarise(
    #pct_cwt_rec = sum(pct_cwt_rec),
    pct_cwt_est_num = sum(pct_cwt_est_num),
    .groups = "drop"
  ) |> 
  #next, calculate the percentage within fishery-year of each tag_code-recovery-percent (thereby bringing along hatchery and "stock"/region)
  #these should sum to 1 per year...
  group_by(run_year) |> 
  mutate(
    #pct_cwt_rec_pct_fishery = pct_cwt_rec / sum(pct_cwt_rec),
    pct_cwt_est_num_pct_fishery = pct_cwt_est_num / sum(pct_cwt_est_num)
  ) |> 
  #now, reaggregate to release region for coarse pattern 
  group_by(
    run_year, 
    #release_location_state, release_location_rmis_region, release_location_rmis_basin,
    release_st_rgn
  ...
  summarise(pct_cwt_est_num_pct_fishery = sum(pct_cwt_est_num_pct_fishery), .groups = "drop") |> 

  #group_by(run_year) |> slice_max(pct_cwt_est_num_pct_fishery, n = 1)
  ggplot(aes(run_year, pct_cwt_est_num_pct_fishery, fill = release_st_rgn)) + geom_col()
...
```{r}
fram_rmis |> 
  filter(StockID==20, run_year==2019, fram > 0, lc_yr > 1) 

#sum across fisheries for a given year
fram_rmis |> 
  select(run_year, sidnm, fidnm, fram, rmis) |> 
  pivot_longer(cols = c(fram, rmis)) |> 
  group_by(run_year, sidnm, name) |> 
  summarise(across(value, sum, na.rm=T), .groups = "drop") |> 
  ggplot(aes(run_year, value, color = name, fill = name)) +
  geom_col(position = "dodge") +
  scale_x_continuous(breaks = 2010:2020) +
  stat_smooth(method="lm", formula=y~1, se=FALSE) +
  facet_wrap(~sidnm, scales="free", labeller = label_wrap_gen(multi_line = F))

#not helpful as is
fram_rmis |> 
  filter(StockID %in% c(20, 38, 96, 164)) |> 
  select(run_year, sidnm, fidnm, fram, rmis) |> 
  pivot_longer(cols = c(fram, rmis)) |> 
  ggplot(aes(run_year, value, color = name, fill = fidnm, group = name)) +
  geom_col(position = position_dodge()) +
  scale_x_continuous(breaks = 2010:2020) +
  stat_smooth(method="lm", formula=y~1, se=FALSE) +
  facet_wrap(~sidnm, scales="free", labeller = label_wrap_gen(multi_line = F))

#interesting but probably not helpful
#shows both the "fog of fram" whereby trivial impacts are assigned broadly
#and the potential for various stock-year-fishery mismatches
fram_rmis |> 
  filter(StockID == 20) |> 
  select(run_year, sidnm, fidnm, fram, rmis) |> 
  pivot_longer(cols = c(fram, rmis)) |> 
  filter(value > 0) |> 
  ggplot(aes(run_year, fidnm, size = value, color = name, group = name)) +
  geom_point(position = position_dodge(width = 0.8)) +
  scale_size_area()

#still visually TMI but easier to see multiyear pattern
fram_rmis |> 
  filter(StockID == 20) |> 
  select(run_year, sidnm, fidnm, fram, rmis) |> 
  pivot_longer(cols = c(fram, rmis)) |> 
  filter(value > 0) |> 
  ggplot(aes(fidnm, value, color = name)) +
  geom_boxplot(outlier.shape = NA) + 
  coord_flip() 
#a little better, but potentially still somewhat misleading where FRAM is very trace
#consider A11 where FRAM assigns something but RMIS is order magnitude higher but still smallish
fram_rmis |> 
  filter(StockID == 20) |> 
  select(run_year, sidnm, fidnm, fram, rmis) |> 
  pivot_longer(cols = c(fram, rmis)) |> 
...
fram_rmis |> 
  filter(StockID == 20, FisheryID %in% fram_ps_spt) |> 
  select(run_year, sidnm, fidnm, fram, rmis) |> 
  pivot_longer(cols = c(fram, rmis)) |> 
  ggplot(aes(fidnm, value, color = name)) +
  geom_boxplot(outlier.shape = NA) + 
  geom_text(aes(label = run_year), size = 2, position = position_dodge2(width = 0.4), alpha = 0.4) +
  coord_flip() 
...
fram_rmis |> 
  ...
  stat_smooth(aes(color = name), method="lm", formula=y~1, se=FALSE) + 
  scale_fill_grey(aesthetics = c("fill", "color")) +
  facet_wrap(~fidnm, scales="free", ncol = 2, labeller = label_wrap_gen(multi_line = F))


#inefficient quick look at functionalizing
map(
  c(20, 38, 96),
  ~fram_rmis |> 
    filter(StockID == .x, FisheryID %in% fram_ps_spt) |> 
    select(run_year, sidnm, fidnm, fram, rmis) |> 
    pivot_longer(cols = c(fram, rmis)) |> 
    ggplot(aes(run_year, value, fill = name)) +
    geom_col(position = position_dodge()) +
